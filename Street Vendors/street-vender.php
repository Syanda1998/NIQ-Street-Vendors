            ...
            <!-- <img src="img/mylogo.png" style="width:200px; height:200px;"></div> -->
                <div class="row my-5">
                    ...
                    if(isset($_POST['btn_delete'])){
                        ...
                        $sql = "DELETE FROM street_vendor_loans WHERE date='$date'";
                        $sql1 = "DELETE FROM street_vendor_collections WHERE loan_id='$date'";
                        $_SESSION['where'] = $date;
                        $_SESSION['sql'] = $sql;
                        $_SESSION['sql1'] = $sql1;
                        $_SESSION['return'] = "<script>location.href = 'street-vender.php?'</script>";
                    ...
                    $mydate = "";
                    $today = 0;
                    $collected = $collected1 = $total = $total_c = 0;
                    $sql4 = "SELECT * FROM street_vendor_collections";
                    $result4 = $conn-> query($sql4);
                    if ($result4-> num_rows > 0){	
                        while($row4= $result4-> fetch_assoc()){
                            $collected = $collected + $row4['amount'];
                             $mydate = $row4['date'];
                            if(substr($mydate, 0, 10) == $date){
                                $today = $today + $row4['amount'];
                            }
                        }
                    }
                   
                    $total_c = $collected + $collected1;
                   $tot_loan = $street_v_loan = $clients_loan = 0;
                   
                   $sql4 = "SELECT * FROM street_vendor_loans";
                   $result4 = $conn-> query($sql4);
                   if ($result4-> num_rows > 0){	
                       while($row4= $result4-> fetch_assoc()){
                           $street_v_loan = $street_v_loan + $row4['total'];
                       }
                   }
                   
                   $tot_loan = $clients_loan + $street_v_loan;
                   
                   $remaining = $tot_loan - $total_c;
                   echo "<h4 style='text-align:center; margin-top:-50px;'>Statistics</h4>";
                   echo "<div class='card' style='width:100%; height:25px;'>";
                   echo "<b>Total Lend Amount = R$tot_loan</b>";
                   echo "</div>";
                                 
                   echo "<div class='card' style='width:100%; height:25px;'>";
                   echo "<b>Total Collected Amount = R$total_c</b>";
                   echo "</div>";
                                
                   echo "<div class='card' style='width:100%; height:25px;'>";
                   echo "<b>Total Remaining Amount = R$remaining</b>";
                   echo "</div>";
                                 
                   echo "<div class='card' style='width:100%; height:25px;'>";
                   echo "<b>Total Collected Today = R$today</b>";
                   echo "</div>";
                   echo "<br>"; echo "<br>";
                    ...
                    echo "<select name='search_id' class='form-control' style='margin-bottom:10px; width:100%; display:inline;'>";
                    echo "<option value='All'>All</option>  <option value='Active'>Active</option>";
                    echo "<option value='Paid off'>Paid off</option>";
                    echo "</select>";

                    echo "<td><button type='submit' style='width: 100%; height:40px;' class='btn' 
                    value='1' name='btn_search'>Search</button></td>";
                    echo "<td><button type='submit' style='width: 100%; height:40px; margin-top:10px;' class='btn' 
                        ...
                        while($row = $result-> fetch_assoc()){
                            ...
                            if ($result1-> num_rows > 0){	
                                ...
                                while($row1 = $result1-> fetch_assoc()){	
                                    ...
                                    if($status=="Paid off"){
                                        echo "<td><button type='submit' class='btn' value='$date' style='width:30%; background-color:lightblue; height:35px; margin-bottom:10px;
                                        display:inline;' disabled >Paid off</button></td>";
                                    }else{
                                        echo "<td><button type='submit' class='btn' value='$date' style='width:30%; background-color:green; height:35px; margin-bottom:10px; 
                                        display:inline;' name='btn_collect'>Collect</button></td>";
                                    }
                                    
                                    echo "<td><button type='submit' class='btn' value='$userid' style='width:30%; margin-left: 10px; background-color:lightgreen; height:35px; margin-bottom:10px;;
                                    display:inline;' name='btn_grant_loan'>Lend</button></td>";
                                    echo "<td><button type='submit' class='btn' value='$date' style='width:30%; margin-left: 10px; background-color:red; height:35px; margin-bottom:10px;;
